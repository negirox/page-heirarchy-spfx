...
import { ErrorHelper, LogHelper, PageFields } from '../utilities';
...
function pagesReducer(state: PagesState, action: Action): PagesState {
  ...
  switch (action.type) {
    ...
    case ActionTypes.PARENT_PAGE_COLUMN_EXISTS:
      const parentPageColumnExistAction: ParentPageColumnExistAction = action as ParentPageColumnExistAction;
      return {
        ...state,
        parentPageColumnExists: parentPageColumnExistAction.payload
      };
    case ActionTypes.CAN_USER_MANAGE_PAGES:
      const canUserManagePagesAction: CanUserManagePagesAction = action as CanUserManagePagesAction;
      return {
        ...state,
...
export function usePageApi(currentPageId: number, pageEditFinished: boolean, context: WebPartContext, treeTop?: number, treeExpandTo?: number): PageApi {
  ...
  useEffect(() => {
    ...
    if (currentPageId && !!spLibGuid) {
      checkIfParentPageExists().then(x=>{
        console.log(x);
      });
      getPagesAsync().then(x=>{
        console.log(x);
      });
    }
  ...
  async function getPagesAsync() :Promise<void>{
    ...
    const pages: IPage[] = [];
    const listo =  await sp.web.lists.getById(spLibGuid)();
    console.log(listo);
  ...
  function buildPageAncestors(allPages: IPage[], pageId: number, ancestorPages: IPage[] = []): IPage[] {
    ...
    if (pageId) {
      const currentPage = allPages.filter((f: { id: number; }) => f.id === pageId);
      if (currentPage.length > 0) {
        ancestorPages.push(currentPage[0]);
        buildPageAncestors(allPages, currentPage[0].parentPageId, ancestorPages);
      }
    }
  ...
  function buildHierarchy(allPages: IPage[], pageId: number): INavLink {
    function recurse(id: number, l: number, ancestorPages: IPage[]): INavLink {
      ...
      return { name: item.title, url: item.url, key: item.id.toString(), links: links, isExpanded: treeExpandTo ? 
        (treeExpandTo >= l) : (ancestorPages.filter((f: { id: number; }) => f.id === id)[0] ? true : false) };
    }
  ...
  const addParentPageField = async ():Promise<void> => {
    await addParentPageFieldToSitePages();
  };
